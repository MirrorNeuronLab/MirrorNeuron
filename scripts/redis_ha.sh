...
SELF_HOST="${MN_REDIS_LOCAL_HOST:-127.0.0.1}"
REDIS_PORT="${MN_REDIS_PORT:-6379}"
SENTINEL_PORT="${MN_REDIS_SENTINEL_PORT:-26379}"
SENTINELS="${MN_REDIS_SENTINELS:-${SELF_HOST}:${SENTINEL_PORT}}"
MASTER_NAME="${MN_REDIS_SENTINEL_MASTER:-mirror-neuron}"
QUORUM="${MN_REDIS_SENTINEL_QUORUM:-1}"
DATA_DIR="${MN_REDIS_DATA_DIR:-/tmp/mirror_neuron_redis}"
REDIS_SERVER="${MN_REDIS_SERVER_BIN:-redis-server}"
REDIS_CLI="${MN_REDIS_CLI_BIN:-redis-cli}"
...
need_bins() {
  command -v "$REDIS_CLI" >/dev/null 2>&1 || {
    echo "redis-cli not found; set MN_REDIS_CLI_BIN" >&2
    exit 1
  }

  if [ "$COMMAND" = "join" ]; then
    command -v "$REDIS_SERVER" >/dev/null 2>&1 || {
      echo "redis-server not found; set MN_REDIS_SERVER_BIN" >&2
      exit 1
    }
  fi
}
