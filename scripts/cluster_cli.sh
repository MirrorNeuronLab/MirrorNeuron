...
CLI_PORT="${MN_CLI_DIST_PORT:-4371}"
SEED_IP=""
...
export MN_NODE_NAME="cli-$(date +%s)-$$@${SELF_IP}"
export MN_NODE_ROLE="control"
export MN_COOKIE="$COOKIE"
export MN_CLUSTER_NODES="$SEED_NODE"
...
export MN_API_PORT="$(find_free_port 4010)"

>&2 echo "Running MirrorNeuron cluster CLI"
>&2 echo "  node: $MN_NODE_NAME"
>&2 echo "  seed: $MN_CLUSTER_NODES"
>&2 echo "  redis: $MN_REDIS_URL"
>&2 echo "  redis ha: $MN_REDIS_HA_MODE"
if [ "$MN_REDIS_HA_MODE" = "sentinel" ]; then
  >&2 echo "  sentinels: $MN_REDIS_SENTINELS"
fi
>&2 echo "  cli dist port: $CLI_PORT"
>&2 echo "  cli api port: $MN_API_PORT"
