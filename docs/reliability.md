...
### Coordinator pinned to the submission node

Job runners and coordinators are now started under a local job supervisor on the runtime node that accepted the submission.

That means:

- if box 2 dies during a job submitted on box 1, the coordinator stays on box 1
- node loss reduces capacity instead of removing the job control plane
...
MirrorNeuron does not yet implement:

- Raft
- quorum writes
- multi-coordinator failover
- leader election for control ownership

Coordinator placement is intentionally simple right now.
