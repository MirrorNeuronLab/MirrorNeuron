...
| `MN_NETWORK_JOIN_TOKEN` | Empty | Join token required by `ClusterService.NetworkHandshake`; the CLI stores it at `~/.mn/network.token` and derives cluster secrets from it. |
...
`ClusterService.NetworkHandshake` is used by `mn join` and `mn add-node` to verify
the token printed by `mn start` or `mn expose-node`. `mn expose-node` runs with
`MN_NETWORK_ONLY=true`; in that mode job, observability, and mutating cluster RPCs
are denied, while cluster/resource summary RPCs remain available for inspection.
