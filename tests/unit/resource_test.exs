defmodule MirrorNeuron.ResourceTest do
  ...
  defmodule Store do
    ...
    def persist_resource_limits(limits), do: Agent.get_and_update(__MODULE__, &persist(&1, limits))
  ...
  test "lists cluster resource totals with default limits" do
    ...
    assert report["totals"] == %{"cpu_cores" => 12, "gpu_count" => 2, "memory_gb" => 24.0}
    assert report["limits"] == %{"cpu" => 100, "gpu" => 100, "memory" => 100}
    assert report["usable"] == %{"cpu_cores" => 12, "gpu_count" => 2, "memory_gb" => 24.0}
  end

  test "sets allowed percentages and applies usable totals" do
    assert {:ok, report} = Resource.set(%{"cpu" => 50, "gpu" => 25, "memory" => "75"})

    assert report["limits"] == %{"cpu" => 50, "gpu" => 25, "memory" => 75}
    assert report["usable"] == %{"cpu_cores" => 6, "gpu_count" => 0, "memory_gb" => 18.0}
