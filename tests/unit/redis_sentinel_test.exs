defmodule MirrorNeuron.Redis.SentinelTest do
  ...
  test "sentinel URL omits database selection and includes sentinel auth" do
    System.put_env("MN_REDIS_SENTINEL_USERNAME", "sentinel-user")
    System.put_env("MN_REDIS_SENTINEL_PASSWORD", "s p@ss")
  ...
  test "resolves primary URL from first healthy sentinel" do
    ...
    System.put_env("MN_REDIS_SENTINELS", "sentinel-a:26379,sentinel-b:26380")
    System.put_env("MN_REDIS_SENTINEL_MASTER", "mirror-neuron")
    System.put_env("MN_REDIS_DB", "2")
    System.put_env("MN_REDIS_URL", "redis://localhost:6379/0")
    System.put_env("MN_REDIS_USERNAME", "worker")
    System.put_env("MN_REDIS_PASSWORD", "secret")
    System.put_env("MN_REDIS_SENTINEL_HOST_MAP", "redis-primary=10.0.0.9")
  ...
  test "resolves primary with rediss scheme inherited from redis URL" do
    System.put_env("MN_REDIS_HA_MODE", "sentinel")
    System.put_env("MN_REDIS_SENTINELS", "sentinel-a:26379")
    System.put_env("MN_REDIS_SENTINEL_MASTER", "mirror-neuron")
    System.put_env("MN_REDIS_DB", "5")
    System.put_env("MN_REDIS_URL", "rediss://localhost:6379/0")
  ...
  test "redis connection URL preserves single mode behavior" do
    ...
    System.put_env("MN_REDIS_URL", "redis://single.example:6379/3")
  ...
  test "sentinel configuration validates" do
    System.put_env("MN_REDIS_HA_MODE", "sentinel")
    System.put_env("MN_REDIS_SENTINELS", "127.0.0.1:26379")
    System.put_env("MN_REDIS_SENTINEL_MASTER", "mirror-neuron")
    System.put_env("MN_REDIS_DB", "0")
    System.put_env("MN_REDIS_WAIT_REPLICAS", "1")
    System.put_env("MN_REDIS_WAIT_TIMEOUT_MS", "100")
  ...
  test "sentinel configuration rejects missing sentinel list" do
    System.put_env("MN_REDIS_HA_MODE", "sentinel")
    System.put_env("MN_REDIS_SENTINELS", "")

    assert_raise ArgumentError, ~r/MN_REDIS_SENTINELS/, fn ->
      MirrorNeuron.Config.validate!()
    end
  end

  test "sentinel configuration rejects invalid mode and negative wait settings" do
    System.put_env("MN_REDIS_HA_MODE", "other")

    assert_raise ArgumentError, ~r/MN_REDIS_HA_MODE/, fn ->
      MirrorNeuron.Config.validate!()
    end
    ...
    assert_raise ArgumentError, ~r/MN_REDIS_WAIT_REPLICAS/, fn ->
      MirrorNeuron.Config.validate!()
    end
    ...
    assert_raise ArgumentError, ~r/MN_REDIS_RECONNECT_ATTEMPTS/, fn ->
      MirrorNeuron.Config.validate!()
    end

    System.put_env("MN_REDIS_RECONNECT_ATTEMPTS", "10")
    System.put_env("MN_REDIS_DB", "-1")

    assert_raise ArgumentError, ~r/MN_REDIS_DB/, fn ->
      MirrorNeuron.Config.validate!()
    end
  end
end
