defmodule MirrorNeuron.AggregatorCompletionTest do
  ...
  test "completes after the configured message count" do
    ...
    {:ok, state1, actions1} =
      Aggregator.handle_message(message(%{"candidate" => 2}), state0, %{})

    refute Enum.any?(actions1, &match?({:complete_run, _}, &1))

    {:ok, state2, actions2} =
      Aggregator.handle_message(message(%{"candidate" => 3}), state1, %{})

    refute Enum.any?(actions2, &match?({:complete_run, _}, &1))

    {:ok, _state3, actions3} =
      Aggregator.handle_message(message(%{"candidate" => 5}), state2, %{})
  ...
  defp message(body), do: Message.new("job-1", "worker", "collector", "result", body)
end
